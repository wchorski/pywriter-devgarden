...
This is how I keep my [pywriter](/vault/../../pywriter.md) vault in sync between my **phone** and other devices. I also use it to sync up with my **Dungeon's & Dragons** vault.
...
## installation
### backend
1. spin up a [CouchDB](/vault/%F0%9F%93%81developer/Home__Lab__%F0%9F%8F%A0/CouchDB.md) server and make it public facing via [Nginx Proxy Manager](/vault/%F0%9F%93%81developer/Home__Lab__%F0%9F%8F%A0/Nginx__Proxy__Manager.md). **vrtmrz** also has an [all in one solution](/vault/https://github.com/vrtmrz/self-hosted-livesync-server) if your are not currently using a **Reverse Proxy**
2. `./compose.yml`
```yaml
version: "3.7"
...
> **Obsidian-livesync** only syncs when 2 client instances of Obsidian are open *(i.e. the mobile app and desktop app)*. I like to think of [vrtmrz/filesystem-livesync](/vault/https://github.com/vrtmrz/filesystem-livesync) as the #headless version of [Obsidian](/vault/%F0%9F%93%81developer/Home__Lab__%F0%9F%8F%A0/Obsidian.md) that makes sure all the other are up-to-date
...
Notice I don't have a `.obsidian` folder in here. The [Obsidian-Livesync](/vault/%F0%9F%93%81developer/Obsidian-Livesync.md) plugin handles syncing other plugins. Then I copy over Themes and settings. 

7. This is an add from me to the [vrtmrz/filesystem-livesync](/vault/https://github.com/vrtmrz/filesystem-livesync) repo. If I can [Docker](/vault/%F0%9F%93%81developer/Home__Lab__%F0%9F%8F%A0/Docker.md) **Compose** it, I will.
`compose.yml`
```yaml
version: "3.7"
...
1. follow the plugin [obsidian-livesync](/vault/https://github.com/vrtmrz/obsidian-livesync)'s instructions
	- add URL of hosted **CouchDB** server
	- [creds](/vault/%F0%9F%93%81shower_thoughts/creds.md) should match "auth" in `config.json`
...
2. follow [Regex to ignore files](/vault/#Regex__to__ignore__files) to ignore correct files / folders also `|📁 Dungeon Master`
